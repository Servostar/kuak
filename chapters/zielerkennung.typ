...
Die Umgebung gibt verschiedene Signale aus, die durch geeignete Sensoren aufgenommen werden können. Im Rahmen dieser Arbeit werden vor allem vier verschiedene Sensortypen beleuchtet: Kameras zur visuellen Erfassung, @LiDAR und @Radar zur Tiefenerkennung und Mikrofone zur akustischen Erfassung.
...
#figure(image("../res/radar.png", width: 65%), caption: [24GHz mmWave Sensor - Radar @radar]) <figure:radar>
...
#figure(image("../res/lidar.png", width: 65%), caption: [Waveshare D500 LiDAR Kit @lidar]) <figure:lidar>
...
#figure(image("../res/microfon.png", width: 65%), caption: [Electret Microphone Amplifier  @microfon]) <figure:microfon>
...
Mit der Aufnahme der Umgebung durch eine passende Sensorik,können zwar potentielle Ziele gesehen, jedoch nicht als Ziele identifiziert werden. Da das Kamerasystem zwei Streams liefert, wird eine Instanz benötigt, welche daraus eine Position von einem Ziel ermittelt. Dabei fällt zum einen das Problem der Identifizierung des Insektes an, als auch die Extraktion der genauen Position davon in Weltkoordinaten.

=== Identifizierung

Um das genaue Ziel auf den Aufnahmen zu filtern, gibt es den klassischen Ansatz der Bildverarbeitung und den Ansatz des maschinellen Lernens.

Bei der klassischen Bildverarbeitung wird über verschiedene Filter die Eigenschaften der Aufnahme hervorgehoben und darauf hin diese mit den Eigenschaften des Zieles verglichen. Zum einen kann über Licht und Farbe eine Fliege von einem Roten Fleck unterschieden werden. Dazu kann die Form, welche über Kanten im Bild ermittelt werden, mit der einer Fliege verglichen werden. Dieses Filtern ist für ein Bild effizient durchzuführen, kann jedoch viel rauschen beinhalten, woraus nicht klar ist, Ob es eine Fliege ist, oder doch nur ein Fleck an der Wand.

Mit einer Klassifikation durch maschinelles Lernen, kann zwar potentiell wesentlich genauer eine FLiege identifiziert werden, ist jedoch auch Rechenintensiver als eine klassische Bildverarbeitung. Zu dem kommt ein benötigtes Training, was zuvor durchgeführt werden muss. maschinelles Lernen eröffnet ebenfalls die Frage nach der Berechnungsgeschwindigkeit. Dieser entstehende Error muss vor allem bei kleinen Zielen bei der Zielerkennung berücksichtigt werden.

Als Ansatz kann dabei eine Mischlösung verwendet werden, wobei eine Initiale Filterung von Potentiellen Fliegen durch eine klassische Bildverarbeitung vorgenommen wird. Potentielle Kandidaten können darauf folgend mithilfe einer Klassifikation in kleinerer Bildgröße kategorisiert werden. So kann der Vorteil einer besseren Klassifikation eingebunden werden, jedoch auch die benötigte Rechenleistung und damit die Benötigte Zeit minimiert werden. Jedoch bleibt das Problem eines Trainings für die Klassifikation, was beschriftete Daten benötigt.@Visalli2021

=== Positionsberechnung

Sobald für beide Aufnahmen ein Ziel Identifiziert wurde, muss daraus folgend eine Position in Weltkoordinaten errechnet werden, sodass die Kinematik sich darauf ausrichten kann. 

Zuvor wurde aus den Aufnahmen die jeweilige Pixelkoordinaten des Zieles berechnet. Die Positionen der Kameras lässt sich aus der Stellung der Kinematik zu den Weltkoordinaten errechnen. Mit diesen Informationen lässt sich durch Triangulation darauf folgend die Position des Zieles in Weltkoordinaten berechnen.

Diese Methode ist jedoch vollständig von den Fehlern der Werte abhängig. Wenn die Identifizierung leicht abweichende Koordinaten liefert, oder die Position der Kameras nicht sauber berechnet wird, können diese Toleranzen sich addieren und das Ziel verfehlen. @imageReconstruction
